...
species_list = ("AHerring", "Sprat", "Cod", "Haddock", "Whiting", "Blue Whiting", "Norway Pout", "Poor Cod", "European Hake", "Monkfish", "Horse Mackerel", "Mackerel", "Common Dab", "Plaice", "Megrim", "Sole", "Boarfish")
```
...
for (a in 1)
  type = 
  first_species <- renamed_df %>% filter(pred_species == fixed(type))
  ggplot(data = first_species, aes(x=log(ppmr)), group=1) + labs(title = type, x="log(PPMR)", y="No. density of observations") + geom_density(colour="red") + theme(plot.title = element_text(size=15))
