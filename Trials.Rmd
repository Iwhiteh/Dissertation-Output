...
We are wanting gradient=1 for proportionality. This is true ($\pm 0.5$) for the species Cod, Common Dab, European Hake, Haddock, Horse Mackerel, Megrim, Monkfish, Norway Pout, Plaice, Poor Cod, Sprat and Whiting. This represents 13 out of the 16 possible predator species, hence the assumption that the predator and prey masses are proportional is a reasonable one, and so we will continue with this assumption.
...
while(i<=length(species_list)){
  ...
  plotted <- ggplot(data = species_df, aes(x=resid(model))) + 
    labs(title = paste("Distribution of residuals:", species_list[i])) +
    geom_density(colour="red")
  plotted
#As a distribution??
  i=i+1
}
#Plot every while loop??
