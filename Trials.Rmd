...
ggplot(species_df2, aes(x = lpred_weight, y = lppmr)) +
  geom_smooth(method = "lm", se = FALSE, color = "lightgrey") +     
  geom_segment(aes(xend = lpred_weight, yend = predicted), alpha = .2,  colour = "blue") +
  geom_point(size=0.5)
...
## Density of lppmr plots

```{r trial d, echo=TRUE}

#Doing a density plot, essentially to find the mean lppmr for each predator species (which is then used for further modelling etc.)
#Find mean for each species by treating pred_species as a fixed effect?

d <- lmer(lppmr ~ pred_species + (1|year), data = renamed_df, REML=FALSE)
d
...
#remove data points where there isn't a fitted(d) value
...
With $\log(\text{ppmr}) = m \times \log(\text{prey mass}) + c$
