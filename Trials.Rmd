...
\begin{itemize}
  \item $i$ is the group
  \item $j$ is the item
  \item $\beta_{0}$ is the intercept term (grand mean)
  \item $S_{0[j]}$ is the effect of the random term (pred_species)
...
$$
  \log(\text{ppmr})_{ij} = \beta_{0} + \beta_{1} \log(\text{prey weight})_{j} + 
                \beta_{2} (\text{predator species})_{j} + A_{0[j]} + B_{0[j]} + C_{0[j]} + \epsilon_{ij}
$$
...
\begin{itemize}
  ...
  \item $\beta_{1}$ is the fixed effect (slope) for $\log(\text{prey weight})$
  \item $\beta_{2}$ is the fixed effect (slope) for $\text{predator species}$
  \item $A_{0[j]}$ is the random intercept for item $i$ (dependent on the haul_id_short term), where $A_{0[j]}$ is normally distributed with a mean of $0$ and a standard deviation $omega_{00}$ (i.e. $I_{0[j]} \sim N(0,\omega_{A[00]}^2)$)
  \item $B_{0[j]}$ dep. on the year term
  \item $C_{0[j]}$ dep. on the ices_rectangle term
  \item $\epsilon_{ij}$ is the residual/error term for each individual item
\end{itemize}

Values:
\begin{itemize}
  \item $\beta_{0} = 3.304181 $ (fixed effects -> intercept -> estimate)
  \item $\beta_{1} = -0.758358$ (fixed effects -> lprey_weight -> estimate)
  \item $\beta_{2} = $ [individual for each pred species??]
  \item $\omega_{A[00]}} = 1.6472$ (random effects -> groups -> ices_rectangle -> std. dev.)   [s.d. for A, haul_id_short]
  \item $\omega_{B[00]}} = 1.4235$ [s.d. for B, year]
  \item $\omega_{C[00]}} = 0.4331$ [s.d. for C, ices_rectangle]
  \item $\epsilon_{ij} = 1.0729$ (random effects -> groups -> residual -> std. dev.)
\end{itemize}
...
Weightings: prey_weight_g, no._prey_per_stmch (do we need??)
