...
species_list <- list("Blue Whiting", "Cod", "Common Dab", "European Hake", "Haddock", "Herring",
...
```{r trialing, echo=TRUE}

species_grad=c()

species_df <- renamed_df %>% filter(pred_species == fixed(species_list[3]))
species_df
grad <- coef(lm((species_df$lpred_weight)~(species_df$lprey_weight)))
species_grad[3] <- grad[2]
species_list[3]
