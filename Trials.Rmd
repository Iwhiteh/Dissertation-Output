---
title: "Predator-Prey Mass Ratios in Aquatic Ecosystems and Fitting Mixed Effects Models to Account to a Given Data Set"
...
A simple linear model is mathematically represented by:
$$
...
(where $\alpha_{0j}$ is normally distributed with a mean of $0$ and a standard deviation $\sigma_{A[00]}$ (i.e. $\alpha_{0y} \sim \mathcal{N} (0, \sigma^2_{A[00]})$)).
* $\beta_{1}$ is the slope term that's dependent on the fixed effect (the predator mass)
* $\epsilon_{iy}$ is the error term, which describes the residuals of the model (where $\epsilon_{ij} \sim \mathcal{N} (0, \sigma^2_{\epsilon})$)
...
(where $\alpha_{1y} \sim \mathcal{N} (0, \sigma^2_{A[10]})$).
...
(where $\alpha_{0y} \sim \mathcal{N} (0, \sigma^2_{A[00]})$).
* $\beta_{1}$ is the slope term that's dependent on the fixed effect (the predator mass)
* $\alpha_{1y}$ is the slope term that's dependent on the random effect (the year)
* $\epsilon_{iy}$ is the error term, which describes the residuals of the model
...
(where $H_{0h}$ is normally distributed with a mean of $0$ and a standard deviation $\sigma_{H[00]}$ (i.e. $H_{0h} \sim \mathcal{N} (0, \sigma^2_{H[00]})$)).
* $\beta_{1}$ is the slope term that's dependent on the fixed effect (the predator mass)
...
(where $Y_{0y}$ is normally distributed with a mean of $0$ and a standard deviation $\sigma_{Y[00]}$ (i.e. $Y_{0y} \sim \mathcal{N} (0, \sigma^2_{Y[00]})$)).
...
($R_{0r} \sim \mathcal{N} (0, \sigma^2_{R[00]})$).
* $R_{1r}$ is the slope term that's dependent on the random effect (the ices_rectangle) (with ($R_{1r} \sim \mathcal{N} (0, \sigma^2_{R[1r]})$).)
...
To note, the coefficients of each random effect are distributed according to: 
$\sigma_{H_{[0h]}}$  $sim \mathcal{N} (0, \sigma^2_{H_{[0h]}})$. However, each group $H$ only has a single standard distribution across every subgroup $h$.
...
When the mixed effects model was applied, I found that there was no estimated gradient value for the predator species `Blue Whiting`. This may be due to some amount of overfitting of the model, or because observations of Blue Whiting only occurred in a limited number of year and ship name subgroups. For conciseness in this project we will therefore ignore the lack of a gradient value for `Blue Whiting` and continue with gradient values only for the remaining 15 predator species
