...
ggplot(data = renamed_df, aes(x=lppmr, colour=pred_species)) + 
  ...
  geom_density(aes(weight = prey_weight_g)) +
  theme(legend.position="none")
...
```
REMOVE THE NUMBERS FROM THE SIDE
...
We can see that the y-intercepts of this mixed effects model are randomly distributed round some central value, and that this y-intercept varies across the years.
...
Here, we have added a normal curve (in dotted black) to the plot using the mean and standard deviation as calculated from the model. Comparing this normal curve to the density of residuals,
