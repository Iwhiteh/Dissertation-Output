...
### Residuals v. log(predator mass)

```{r residuals v. lpred_weight, echo=TRUE, message=FALSE}
ggplot(herring_model, aes(x = lpred_weight, y = .resid)) +
  geom_point(colour="red", size=0.5) +
  geom_smooth() + 
  labs(title='Residuals v. log(predator mass): Herring', x='log(predator mass)', y='Residuals')
```
...
$$
  \log(\text{ppmr})_{ij} = \beta_{0} + \alpha_{0j} + 
        \beta_{1} \log(\text{predator mass})_i + \epsilon_{ij}
$$
...
$$
  \log(\text{ppmr})_{ij} = \beta_{0} + 
        (\beta_{1} + \alpha_{1j}) \log(\text{predator mass})_i + \epsilon_{ij}
$$
...
\log(\text{ppmr})_{ih} = \beta_{0} + H_{0h} + 
        (\beta_{1} + H_{1h}) \log(\text{predator mass})_i + \epsilon_{ih}
$$
...
$$  
  \log(\text{ppmr})_{ihy} = \beta_{0} + H_{0h} + Y_{0y} +
        (\beta_{1} + H_{1h} + Y_{1y}) \log(\text{predator mass})_i + \epsilon_{ihyr}
$$
...
#  subset(herring_df, !(herring_df$lppmr %in% lppmr))
#  ggplot(aes(x=lpred_weight, y=lppmr)) + 
#  labs(title='log(PPMR) v. log(predator mass) plot for model b: Herring', 
#         x='log(predator mass)', y='log(ppmr)') +
#  geom_point(size=0.001, colour="red") 
...
ggplot(aes(x=lpred_weight), data=herring_df) +
  labs(title='log(PPMR) v. log(predator mass) plot for model b: Herring', 
         x='log(predator mass)', y='log(ppmr)') +
  geom_point(aes(y=fitted(a)), colour="blue") +
  geom_point(aes(y=fitted(b)), colour="red")

ggplot(data=herring_df) +
  labs(title='Density plot of the residuals: Herring', x='Residuals', y='Density') +
  geom_density(aes(x=resid(a)), colour="blue") +
  geom_density(aes(x=resid(b)), colour="red")
#  geom_density(aes(x=resid(c)), colour="green") +
#  geom_density(aes(x=resid(d)), colour="black")
```
...
* $P_{0p}$ is the random intercept term for predator species $p$
* $\beta_{1}$ is the slope term that's dependent on the fixed effect (the predator mass)
...
plot(density(resid(all)), main="Density plot of the residuals",
     xlab="Residuals", col="red")

ggplot(data=renamed_df) +
  geom_density(aes(x = resid(all)),colour="red") +
...
$\beta_{1}$   | the fixed effect (slope) for $\log(\text{pred mass})$ | 0.2307  | fixed effects -> lpred_weight -> estimate
