...
herring_df %>% 
  ...
  labs(title='log(PPMR) v. log(predator weight) plot for model a: Herring', 
         x='log(predator weight)', y='log(ppmr)') +
...
herring_df %>% 
  ...
  ggplot(aes(x=lpred_weight, y=lppmr)) + 
  labs(title='log(PPMR) v. log(predator weight) plot for model b: Herring', 
         x='log(predator weight)', y='log(ppmr)') +
  geom_point(size=0.001, colour="red") 
...
herring_df %>% 
  ...
  ggplot(aes(x=lpred_weight, y=lppmr)) + 
  labs(title='log(PPMR) v. log(predator weight) plot for model c: Herring', 
         x='log(predator weight)', y='log(ppmr)') +
  geom_point(size=0.001, colour="red") 
...
* $D_{0[j]}$ is the random intercept term for each observation $j$ - this is dependent on the predator species
...
```{r trial d, echo=TRUE}
d <- lmer(lppmr ~ lpred_weight + (1+lpred_weight|haul_id_short) + (1+lpred_weight|year) +
...
herring_df %>% 
  ...
  ggplot(aes(x=lpred_weight, y=lppmr)) + 
  labs(title='log(PPMR) v. log(predator weight) plot for model d: Herring', 
         x='log(predator weight)', y='log(ppmr)') +
  geom_point(size=0.001, colour="red") 

herring_df %>% 
  mutate(lppmr= fitted(d)) %>% 
  ggplot(aes(x = resid(d))) +
  geom_density(colour="red") +
  labs(title='Density plot of the residuals for model d: Herring', x='Residuals', y='Density')
...
renamed_df %>% 
  mutate(lppmr= fitted(c)) %>% 
  ggplot(aes(x=lpred_weight, y=lppmr)) + 
  labs(title='log(PPMR) v. log(predator weight) plot for model c', 
         x='log(predator weight)', y='log(ppmr)') +
  geom_point(size=0.001, colour="red") +
