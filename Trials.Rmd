...
$$
    \text{no. prey per observation} = \frac{\text{no. of stomachs sampled}}{\text{total no. of prey}}
$$
This is an approximation of the number of prey per observation for some specific predator, and is called 'no._prey_per_stmch'.
...
```{r log(prey weight) v. number of prey per observation}
...
ggplot (data = renamed_df, aes(x=lprey_weight, y=no._prey_per_stmch)) + 
  labs(title = "Number of prey per observation v. log(prey weight) - separated by all ship names",
       x="log(prey weight)", y="Number of prey per observation") + 
...
ggplot (data = interesting_haul, aes(x=lprey_weight, y=no._prey_per_stmch)) + 
  labs(title = "Number of prey per observation v. log(prey weight) 
...
3. lots of the same no. of prey per observation observations for HIDDINK.(27180)
...
```{r log(prey weight) v. log(pred weight), message=FALSE}
#message=FALSE means some output messages aren't printed in the knitted HTML document
...
#this also creates a linear model of log(prey weight) against log(predator weight) for each individual predator species, and extracts the standard error of the line of best fit for this linear model, storing this in an array called 'uncert'
...
i=0
while(i<=length(species_list)){
  paste(print(species_list[i]))
  paste(print(species_grad[i] - uncert[i]))
  paste(print(species_grad[i] + uncert[i]))
  i=i+1
}
```
...
Therefore, we can continue with the assumption that there is a difference in the mean of log(ppmr) of the square of the standard deviation when we weight by number of prey per observation and prey biomass.
...
1. green and dotted: by number of prey per observation
...
kable(coef(one)$pred_species)
#This outputs the intercept and slope terms for each predator species for the fitted model
```
...
$$
  \log(\text{ppmr})_{ij} = (\beta_{0} + \alpha_{0j}) + 
        \beta_{1} \log(\text{predator mass})_{i} + \epsilon_{ij}
$$

Where:

* $i$ is the index of the individual observation
* $j$ is the index of the predator species
* $\beta_{0}$ is the overall intercept term
* $\alpha_{0j}$ is the random intercept term for predator species $j$
(where $\alpha_{0j}$ is normally distributed with a mean of $0$ and a standard deviation $\sigma_{A[00]}$ (i.e. $\alpha_{0j} \sim (0, \sigma^2_{A[00]})$)).
* $\beta_{1}$ is the slope term that's dependent on the fixed effect (the predator mass)
...
(where $\alpha_{1j}$ is normally distributed with a mean of $0$ and a standard deviation $\sigma_{A[10]}$ (i.e. $\alpha_{1j} \sim (0, \sigma^2_{A[10]})$)).
* $\epsilon_{ij}$ is the error term, which describes the residuals of the model
...
(where each random intercept term is normally distributed with a mean of $0$ and a standard deviation $\sigma_{[00]}$ (i.e. $sim (0, \sigma^2_{[00]})$)).
...
$\sigma_{H[0h]}$ |  s.d. for the intercept term $H_{0h}$, dependent on the haul_id_short | 1.6711
$\sigma_{H[1h]}$ |  s.d. for the slope term $H_{1h}$, dependent on the haul_id_short | 0.2007
$\sigma_{Y[0y]}$ |  s.d. for the intercept term $Y_{0y}$, dependent on the year | 1.0074
$\sigma_{Y[1y]}$ |  s.d. for the slope term $Y_{1y}$, dependent on the year| 0.1261
$\sigma_{R[0r]}$ |  s.d. for the intercept term $R_{0r}$, dependent on the ices_rectangle | 0.8649
$\sigma_{R[1r]}$ |  s.d. for the slope term $R_{1r}$, dependent on the ices_rectangle | 0.1498
$\sigma_{P[0p]}$ |  s.d. for the intercept term $P_{0p}$, dependent on the predator species | 0.7790
$\sigma_{P[1p]}$ |  s.d. for the slope term $P_{1p}$, dependent on the predator species | 0.1699 
$\epsilon_{ihyr}$ | the s.d. of the residual/error term | 1.8935

## Separated by predator species plots {.tabset}

Separating the plots out over the predator species provides the following information:

### log(PPMR) v. log(predator mass) plot

```{r}
renamed_df %>% 
  mutate(lppmr= fitted(final)) %>% 
  ggplot(aes(x=lpred_weight, y=lppmr, color=pred_species)) + 
  labs(title='log(PPMR) v. log(predator mass) plot - separated by species', 
         x='log(predator mass)', y='log(ppmr)') +
  facet_wrap(~pred_species, scale="free_y") +
...
### Density of log(prey mass) observations
```{r}
ggplot(mackerel_df, aes(x=lprey_weight)) +
  geom_density(aes(weight = no._prey_per_stmch), color="lightseagreen") +
  labs(x="log(prey mass)")
```

### Fitted log(PPMR) v. log(predator mass) plot
```{r}
mackerel_df %>% 
  mutate(lppmr= fitted(c)) %>% 
  ggplot(aes(x=lpred_weight, y=lppmr)) + 
  labs(x='log(predator mass)', y='log(ppmr)') +
  geom_point(size=0.001, colour="lightseagreen") 
```
