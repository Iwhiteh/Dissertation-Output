...
```{r loading, message=FALSE, include=FALSE, echo=TRUE}
...
```{r cleaning smaller, message=FALSE, include=FALSE, echo=TRUE}
renamed_df = df %>% 
...
## Trialing differnt models

```{r mixed, echo=TRUE}
a <- lmer(lppmr ~ pred_species + prey_weight_g + (1|haul_id_short), data = renamed_df, REML=FALSE)
...
b <- lmer(lppmr ~ pred_species + prey_weight_g + (1|haul_id_short) + (1|ices_rectangle), data = renamed_df, REML=FALSE)

c <- lmer(lppmr ~ pred_species + prey_weight_g + (1|haul_id_short) + (1|ices_rectangle) + (1|year), data = renamed_df, REML=FALSE)

d <- lmer(lppmr ~ pred_species + prey_weight_g + (1|haul_id_short) + (1|ices_rectangle) + (1|year) + (1|no._prey_per_stmch), data = renamed_df, REML=FALSE)
...
By comparing the AIC and BIC values, we can see that the final model is the most effective.
...
Random effects: haul_id_short, year, ices_rectangle, no. prey per stomach (?)
