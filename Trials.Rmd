...
while(j<=length(species_list)){
  species_df <- renamed_df %>% filter("Predator_species" == fixed(species_list[j]))
...
This means that the gradient of 12 out of the possible 16 species is equal to $0 \pm 0.5$. However, this amount of error is fairly large, so we have not yet supported the idea that the assumption of independence between the $log\text{ppmr}$ and $\log(\text{predator weight})$ can be upheld for this data set.
...
ggplot(data=species_df, aes(lpred_weight, lppmr)) + 
  ...
  scale_linetype_manual(values = c(rep("solid", 2), rep("dashed", 2), "twodash"))
  theme_classic()
