...
ship_erroneous <- data.frame("Type"=c("y proportional to exp(-x)", "Single prey weight"
                                      , "Same number of prey per stomach", "Same data"),
                        "Number of observations"=c(type1, type2, type3, type4))
...
paste("slope of the log(prey weight) v. log(predator weight) line of best fit:", slope[2])
...
We are wanting gradient=1 for proportionality. This is true ($\pm 0.5$) for the species:
...
This represents 13 out of the 16 possible predator species, hence the assumption that the predator and prey masses are proportional is a reasonable one, and so we will continue with this assumption.

This assumption of proportionality allows us to use the PPMR (predator prey mass ratio) for further analysis in this project.
...
paste("Mean value of log(ppmr), weighted by the number of prey per stomach:", no_herringmean)
...
#resizes the plot to make it fit in the output html file in a neater way
...
The mean is shifted by 6.911843 ($= 13.54102 - 6.629177$).
...
ggplot(data=renamed_df, aes(lpred_weight, lppmr)) + 
  ...
  labs(title = "log(PPMR) v. log(predator weight) plot", x="log(Predator weight)", y="log(PPMR)") + 
...
This means that the gradients of 12 out of the possible 16 species $= 0 \pm 0.5$. However, this amount of error is fairly large, so we have not yet proved that the assumption of independence can be upheld for this data set.
...
## Residuals of the lppmr v. lpred weight

As an alternative proof, we can look at the residuals of these plots to see if the residuals are normally distributed. If the residuals of some plot are normally distributed, it means that the assumed linear model is valid, i.e. if the residuals are normally distributed then the assumption that the PPMR and predator species are independent can be upheld.
...
ggplot(herring_df, aes(x = lpred_weight, y = lppmr)) +
  ...
  geom_segment(aes(xend = lpred_weight, yend = res), alpha = .2,  colour = "red") +
  geom_point(size=0.5)
...
ggplot(herring_model, aes(x = .fitted, y = .resid)) +
  ...
  labs(title='Plot of the residuals against the fitted values', x='Fitted Values', y='Residuals')
...
We can see that for the predator species Herring, the residuals do not particularly look like they are normally distributed.
...
Therefore, it could be said that the assumption of a linear relation between the $\log(PPMR)$ and predator weight holds for these 7 species. However, none of the proposed normal distributions are that strongly reliable, hence it is a more likely assumption to choose that there is in fact some dependence between the $\log(PPMR)$ and predator weight.
...
With $\log(\text{ppmr}) = m \times \log(\text{predator weight}) + c$
