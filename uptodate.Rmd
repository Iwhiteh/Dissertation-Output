...
df_smaller_fixed <- df_smaller[df_smaller$indiv_prey_weight != 0, ]

renamed_df_small = df_smaller_fixed %>% 
...
ggplot(data = renamed_df_small, aes(indiv_prey_weight, no._prey_per_stmch)) + labs(title = "prey weight v. number of prey per stomach", x="prey weight (g)", y="No. of prey per predator stomach", , caption = "Playing around with data to see any specific correlations; what is the distribution of the weight of prey recorded") + geom_point(size=0.75)

print("Playing around with data to see any specific correlations; what is the distribution of the weight of prey recorded")

ggplot(data = renamed_df_small, aes(log(indiv_prey_weight), no._prey_per_stmch)) + labs(title = "log(prey weight) v. number of prey per predator stomach", x="log(prey weight)", y="No. of prey", caption = "Identifying any 'interesting' looking outputs") + geom_point(size=0.5)
#Some interesting results --> introduce colours to show ships  

renamed_df_small$'haul_id_short' <- gsub("\\-.*", "", renamed_df_small$'haul_id')
#rename haul_id values -> separate by ship names (e.g. CLYDE) rather than complete id (e.g. CLYDE-1935-6)

#separated by ship names
ggplot(data = renamed_df_small, aes(log(indiv_prey_weight), no._prey_per_stmch, colour=haul_id_short)) + labs(title = "log(prey weight) v. number of prey per stomach - separate ship names", x="log(prey weight)", y="No.of prey", caption = "Identifying the data points by the boats they were sampled by; shows data lying on a surprsingly nice curve for one of the boats") + geom_point(size=0.5) + theme(plot.title = element_text(size=10))

ggplot (data = renamed_df_small, aes(x=log(indiv_prey_weight), y=no._prey_per_stmch)) + labs(title = "log(prey weight) v. number of prey per stomach", x="log(prey weight)", y="No. prey per stomach", caption="Separating the graph for each boat to identify any specifically suspicious data points: note y prop. to e^-x relation for END04, lots of observations for single weights for LANCELOT; lots of the same no. of fish observations for BULLEN") + geom_point(size=.2, colour="red") + facet_wrap(~haul_id_short, scale="free_y") + theme(strip.text = element_text(size = 5))

ggplot (data = renamed_df_small, aes(x=log(indiv_prey_weight))) + labs(title = "log(prey weight) v. number of prey per stomach - histogram weighted by no. of prey", x="log(prey weight)", y="No. prey per stomach", caption="Separating the graph for each boat to identify any specifically suspicious data points: note y prop. to e^-x relation for END04, lots of observations for single weights for LANCELOT; lots of the same no. of fish observations for BULLEN") + facet_wrap(~haul_id_short, scale="free_y") + theme(strip.text = element_text(size = 5)) + geom_density(aes(weight = no._prey_per_stmch), colour="red")
```
...
ggplot (data = renamed_df_small, aes(indiv_prey_weight, pred_weight_g)) + geom_point(size=0.5) + labs(title = "Predator v. prey mass plot", x="Prey mass (g)", y="Predator mass (g)", caption="Attempting to find a link between the predator mass and the prey mass") 
#mass since measured in g

ggplot(data = renamed_df_small, aes(log(indiv_prey_weight), log(pred_weight_g))) + geom_point(size=0.5) + labs(title = "log(Predator mass) v. log(prey mass) plot", x="log(Prey mass)", y="log(Predator mass)", caption = "Using log() to see proportionality of the axes, slope of added line should = PPMR") + stat_smooth (method='lm', se=FALSE)

slope <- coef(lm(log(pred_weight)~log(prey_weight)))
slope
#gives the slope of the log(pred) v. log(prey) line of best fit

ggplot(data = renamed_df_small, aes(log(indiv_prey_weight), log(pred_weight_g))) + labs(title = "log(pred. mass) v. log(prey mass) separated by predator species", x="log(prey mass)", y="log(pred. mass)", caption = "Looking to find correlation between the masses for each individual species; the slope should intersect the y-axis at 0, else our idea for PPMR calculation (pred mass is prop. to prey mass) is incorrect.") + geom_point(size=0.2) + facet_wrap(~pred_species, scale="free_y", ncol=3) + theme(strip.text = element_text(size = 2))

ggplot(data=renamed_df_small, aes(log(pred_weight_g), log(ppmr))) + geom_point(size=0.5) + labs(title = "log(pred mass) v. log(ppmr) plot", x="log(Pred mass)", y="log(PPMR)") + stat_smooth (method='lm', se=FALSE)
                                                                                           
#caption = "The slope is not =1, so the PPMR is indept. of pred. mass (as desired)")                      

slope2 <- coef(lm(log(PPMR)~log(pred_weight)))
slope2
...
ggplot(data = renamed_df_small, aes(log(ppmr), no._prey_per_stmch)) + labs(title = "Scatter plot: log(PPMR) v. no. of prey per stomach", x="log(PPMR)", y="no. prey per stomach", caption = "Trying to find the most common PPMR over all the fish") + geom_point(size=0.5)
#gives the most common PPMR for fish

ggplot(data = renamed_df_small, aes(x=log(ppmr))) + labs(title = "Histogram: log(PPMR) v. number of observations", x="log(PPMR)", y="No. observations", caption = "no. observation for each value of log(PPMR), i.e. seeing more clearly what the most common PPMR is across all fish species") + geom_histogram(binwidth=.5, colour="black", fill="white")
...
ggplot(data = renamed_df_small, aes(x=log(ppmr), y=no._prey_per_stmch), group=1) + labs(title = "Scatter plot: log(PPMR) v. number density of prey separated by species", x="log(PPMR)", y="No. prey per stomach", caption = "looking for the most common PPMR for each individual species") + geom_point(size=0.2, colour="red") + facet_wrap(~pred_species, scale="free_y", ncol=3) + theme(strip.text = element_text(size = 5))

#Changed into a density plot
ggplot(data = renamed_df_small, aes(x=log(ppmr)), group=1) + labs(title = "Histogram: log(PPMR) v. number of observations, separated by species", x="log(PPMR)", y="No. observations", caption = "same idea as above but using a histogram to show most common PPMR per species in a clearer way - the most common should be the one with the single largest bar") + geom_histogram(binwidth=.5, colour="blue", fill="white") + facet_wrap(~pred_species, scale="free_y") + theme(strip.text = element_text(size = 5))

first_species <- renamed_df_small %>% filter(pred_species == fixed("Clupea harengus"))
#separate data set of a single species
