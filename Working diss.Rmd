...
ggplot(data = df_smaller, aes(log(indiv_prey_weight), log(pred_weight_g))) + labs(title = "log(pred. mass) v. log(prey mass) separated by predator species", x="log(prey mass)", y="log(pred. mass)", caption = "Looking to find correlation between the masses for each individual species; the slope should intersect the y-axis at 0, else our idea for PPMR calculation (pred mass is prop. to prey mass) is incorrect.") + geom_point(size=0.2) + facet_wrap(~pred_species, scale="free_y") + theme(strip.text = element_text(size = 5)) + stat_smooth(method='lm', se=FALSE)
...
ggplot(data = df_smaller, aes(x=log(ppmr)), group=1) + labs(title = "Histogram: log(PPMR) v. number of observations, separated by species", x="log(PPMR)", y="No. density of prey", caption = "same idea as above but using a histogram to show most common PPMR per species in a clearer way - the most common should be the one with the single largest bar") + geom_histogram(binwidth=.5, colour="blue", fill="white") + facet_wrap(~pred_species, scale="free_y") + theme(strip.text = element_text(size = 5))

first_species <- df_smaller %>% filter(pred_species == fixed("Clupea harengus"))
#separate data set of a single species

ggplot(data = first_species, aes(x=log(ppmr)), group=1) + labs(title = "Histogram: log(PPMR) v. number density of prey separated by species - Clupea harengus", x="log(PPMR)", y="No. density of prey") + geom_histogram(binwidth=.5, colour="black", fill="white") + theme(plot.title = element_text(size=15))
```
