...
species_list <- list("Herring", "Sprat", "Cod", "Haddock", "Whiting", "Blue Whiting", 
                     "Norway Pout", "Poor Cod", "European Hake", "Monkfish", "Horse Mackerel", 
                     "Mackerel", "Common Dab", "Plaice", "Megrim", "Sole", "Boarfish")
...
$$
This is an aproximation of the number of prey per stomach for some specific predator, and is called '
no._prey_per_stmch'.
...
Here, we are looking for the most common log(PPMR) for each individual species. This will be done by plotting log(PPMR) against the density of points, and weighting observations on different variables. 
...
$$
  \text{expected prey biomass weighted mean} = \text{no. of prey weighted mean} - (\text{no. of prey standard deviation})^2
$$

Hence, the expected result is:

$$
  \text{actual prey biomass weighted mean} = 13.54102 - (2.63330496940788)^2 = 6.60672493809
$$
...
```{r log(prey weight) v. log(pred weight)}
...
# method='lm' ensures it's a straight line, i.e. a linear model
# se=FAlSE creates only a single line, with no error of margin included

#scale_fill_continuous creates a density colour scale, meaning that areas with a high density of points are yellow and areas with a low density of points are darker in colour
...
Attempting to find a link between the predator mass and the prey mass. Log() of each axis is used to see the proportionality of the axes, where the slope of the added line should = 1. This is because:

$$
 \text{log(pred weight)} = m * \text{log(prey weight)} + c
$$
...
$$
  \text{pred weight} = exp^{m*\text{log(preyweight)}} + log(D)
$$
where log(d) = c. 
...
$$
  \text{pred weight} = \text{prey weight}^m * D
$$
...
#for i in length(pred_species)
 # species_df <- renamed_df %>% filter(pred_species == fixed("Poor Cod"))
  #grad <- coef(lm(renamed_df$lpred_weight~renamed_df$lprey_weight))
  #paste("slope of the log(pred) v. log(prey) line of best fit:", grad[2])
...
$$
    \text{log(pred mass)} = m*\text{log(prey mass)} + c
$$
