...
#introducing a density plot for later
```
...
ggplot(data=species_df, aes(lpred_weight, lppmr)) + 
  ...
  facet_wrap(~pred_species, scale="free_y") + 
  stat_smooth(aes(weight=no._prey_per_stmch), method='lm', colour='blue', se=FALSE) +
  stat_smooth(method='lm', colour='red', se=FALSE) +
  stat_smooth(aes(weight=prey_weight_g), method='lm', colour='green', se=FALSE)
...
Looking at just the predator species 'poor cod'. The LOBF are weighted by different variables
