...
#Creates a new dataframe (called 'renamed_df') with these replaced names
...
```{r prey weight v. pred weight, warning=FALSE}
...
```{r pred weight v. ppmr, warning=FALSE}
...
ggplot(data=species_df, aes(lpred_weight, lppmr)) + 
  ...
  scale_colour_manual(name="Weightings",
                      breaks=c('by no. of prey in stomach', 'no weighting', 'by prey bio'),
...
#linerange draws a point range between an upper and lower limit for the line, and the mean for the point

print(paste("slope of the log(ppmr) v. log(pred_weight) line of best fit for poor cod:"))
...
print(paste("i) Weighted by no prey in the stomach:", perstmch_weighting[2]))

#no weighting
no_weighting <- coef(lm(species_df$lppmr~species_df$lpred_weight))
print(paste("ii) No weighting in the calculation:", no_weighting[2]))

#weighted by prey biomass
biomass_weighting <- 
  coef(lm(species_df$lppmr~species_df$lpred_weight, weight=species_df$prey_weight_g))
print(paste("iii) Weighted by prey biomass:", biomass_weighting[2]))
...
Looking at just the predator species 'poor cod'. The LOBF (line of best fit) is weighted by different variables
i) blue: by number of prey per stomach
ii) red: no weighting in the LOBF
iii) green: by prey biomass

When weighted by prey biomass, the gradient is equal to zero for 2 s.f.. Hence, the approximation is true when looking at a prey biomass weighting.
